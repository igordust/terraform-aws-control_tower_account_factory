...
resource "aws_codepipeline" "s3_account_provisioning_customizations" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        S3Bucket             = aws_s3_bucket.account_provisioning_customizations[0].bucket
        S3ObjectKey          = local.s3_archive_filename
        PollForSourceChanges = false
      }
    }
  ...
  stage {
    ...
    action {
      ...
      provider         = "CodeBuild"
      input_artifacts  = ["account-provisioning-customizations"]
      output_artifacts = ["account-provisioning-customizations-terraform"]
      version          = "1"
      run_order        = "2"
      configuration = {
        ProjectName = aws_codebuild_project.account_request.name
...
resource "aws_cloudwatch_event_rule" "s3_account_provisioning_customizations" {
  count       = local.vcs.is_s3 ? 1 : 0
  name        = "aft-account-provisioning-customizations-codepipeline-trigger"
  description = "Trigger CodePipeline upon upload to S3"
  ...
  event_pattern = <<EOF
{
  ...
  "detail": {
    ...
    "requestParameters": {
      "bucketName": [
          "${aws_s3_bucket.account_provisioning_customizations[0].bucket}"
      ],
      "key": [
          "${local.s3_archive_filename}"
      ]
